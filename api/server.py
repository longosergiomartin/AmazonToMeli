...
_BOOKMARKLET_CODIGOS = (
    ...
    "return (10-s%10)%10===v;}"
    "var res=[],i=0,corto=false,fallas=0,sincodigo=0,ultimo='';"
    ...
    "else{sincodigo++;}"
    "}).catch(function(e){fallas++;ultimo=String((e&&e.message)||e);})"
    ".then(function(){setTimeout(paso,P);});}"
    "function fin(){"
    "caja.textContent='Listo: '+res.length+' código(s). Guardando…';"
    # Si no salió nada, se dice POR QUÉ: antes se tragaba los errores en
    # silencio y no había forma de saber si Amazon bloqueó, si las fichas no
    # traen el código o si el botón ni siquiera pudo pedir la página.
    "if(!res.length){"
    "var d='No encontré ningún código.\\n\\n'"
    "+'Fichas leídas, sin código en la ficha: '+sincodigo+'\\n'"
    "+'Fichas que no pude leer: '+fallas+(ultimo?'  → '+ultimo:'')+'\\n';"
    "if(corto)d+='\\nAmazon pidió verificación: probá más tarde.';"
    "else if(fallas&&!sincodigo)d+='\\nEl navegador no dejó pedir las páginas. "
    "Probá parado en una ficha de producto (una URL con /dp/), no en la portada.';"
    "alert(d);caja.remove();return;}"
